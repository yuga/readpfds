...
module RealTimeDequeP (ConstNum : CONSTNUM) : RDEQUEPS = struct
  module S = SmallStream

  type 'a q = int * 'a S.stream * 'a S.stream
            * int * 'a S.stream * 'a S.stream
  ...
  let size (lenf, f, sf, lenr, r, sr) = lenf + lenr
  ...
  let print_queue print_a show (lenf, f, sf, lenr, r, sr) =
...
struct
  include RealTimeDequeP (ConstNum)
